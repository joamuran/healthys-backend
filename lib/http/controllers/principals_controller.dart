...
class PrincipalsController {
  ...
  Future<Response> getPrincipals(Request request) async {
    // Cridem al cas d'ús per obtenir els plats principals
    List<Principal> principals = await useCases.getPrincipals.execute();

    // Convertim la llista de plats a JSON
    final jsonResponse = _toJSON(principals);
  ...
  Future<Response> getPrincipalsPerTipus(Request request) async {
  ...
  List<dynamic> _toJSON(List<Principal> principals) {
    return principals
        .map(
          (principal) => {
            'name': principal.name,
            ...
            'calories': principal.calories,
            'dietType': principal.dietType,
            'additionalInfo': principal.additionalInfo,
            'img': principal.img,
          },
        )
        .toList();
